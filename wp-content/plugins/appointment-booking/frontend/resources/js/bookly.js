          ...
          success   : function (response) {
            if (response.success) {
              ...
              var $select_category = $('.ab-select-category', $container),
                ...
                $select_type   = $('.ab-select-type', $container),
...
if(staff_id == 1)
{
	...
	if($select_category.val()==1 && $select_service.val()==1)
		{
	...
	else if($select_category.val()!=1)
		{ 
					$('.ab-Postnummer', $container).hide();
		
		}
		
		$select_nop.on('keyup', function() {
		...
		var nop = this.value;
		var total_costo = 0;
		var costo_impuesto = 0;
		
		if (nop<=0) {
		total_costo = 0;
		}else if (nop>0 && nop <=40) {
		total_costo = 1200;
		}else if(nop>=41 && nop <=70) { 
		total_costo = 1320;
		}else if(nop>=11 && nop <=90) { 
		total_costo = 1440;
		}else if(nop>=91 && nop <=100) { 
		total_costo = 1600;
		}else if(nop>=101 && nop <=150) { 
		total_costo = 15*nop;
		}else if(nop>=151) { 
		total_costo = 14*nop;
		}

costo_impuesto = 2*total_costo;
$(".monto").html(total_costo);
$(".montoimp").html(costo_impuesto);
$('.costo', $container).show();
...
else if(staff_id == 2)
{
	...
	if($select_category.val()==1 && $select_service.val()==1)
		...
		total_costo = 15*nop-246;
		}else if(nop>=151) { 
		total_costo = 14*nop-350;
		}
...
///VALIDAR SI ES CATEGORIA 1 y SERVICIO 1							
...
///VALIDAR SI ES CATEGORIA 1 y SERVICIO 1 y STAFF=FRECUENCIA 1 	
    ...
    function hideByAttributes() {
      ...
      if (!Options.attributes.show_type) {
        $('.ab-type', $container).hide();
      }
