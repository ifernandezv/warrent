jQuery(function($) {
    ...
    $('#reportrange_purchases').daterangepicker(
        ...
        function (start, end) {
            ...
            $range_purchases
                ...
                .html(start.format(BooklyL10n.date_format) + ' - ' + end.format(BooklyL10n.date_format));
        }
    );
    ...
    $('#send_test_sms').on('click', function(e){
        ...
        $.ajax({
            url         : ajaxurl,
            data        : { action: 'ab_send_test_sms', 'phone_number': (BooklyL10n.intlTelInput.use) ? $phone_input.intlTelInput('getNumber') : $phone_input.val() },
            dataType    : 'json',
            xhrFields   : { withCredentials: true },
            crossDomain : 'withCredentials' in new XMLHttpRequest(),
            success     : function (response) {
