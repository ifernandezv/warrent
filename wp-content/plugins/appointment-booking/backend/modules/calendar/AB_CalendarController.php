...
class AB_CalendarController extends AB_Controller
{
    ...
    public function index()
    {
        ...
        $this->render( 'calendar', compact( 'custom_fields' ) );
    }
    ...
    public function executeGetDataForAppointment()
    {
        ...
        if ( $appointment->load( $this->getParameter( 'id' ) ) ) {
            ...
            $info = AB_Appointment::query( 'a' )
                ->select( 'ss.capacity AS max_capacity, ss.price as ser_price,SUM( ca.number_of_persons ) AS total_number_of_persons, a.staff_id, a.service_id, a.start_date, a.end_date' )
			...
			$response['data']['ser_price']          	 = $info['ser_price'];
                ...
                if ( $staff_service->get( 'capacity' ) == 1 ) {
                    ...
                    if ( ! empty ( $customer_appointments ) ) {
                        ...
                        foreach ( array( 'name', 'phone', 'email' ) as $data_entry ) {
                            ...
                            if ( $entry_value ) {
                                $description[] = '<div class="fc-employee">' . esc_html( $entry_value ) . '</div>';
                            }
                        }

                        foreach ( $customer_appointments[0]->getCustomFields() as $custom_field ) {
                            $description[] = '<div class="fc-notes">' . wp_strip_all_tags( $custom_field['label'] ) . ': ' . esc_html( $custom_field['value'] ) . '</div>';
                        }
                    }
                } else {
                    $signed_up = 0;
