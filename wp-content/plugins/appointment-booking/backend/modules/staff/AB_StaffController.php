...
class AB_StaffController extends AB_Controller
{
    ...
    public function executeEditStaff()
    {
        ...
        if ( isset( $_SESSION['bookly_updated'] ) ) {
            ...
            $this->updated = true;
        } elseif ( isset ( $_SESSION['google_calendar_error'] ) ) {
            $errors[] = __( 'Calendar ID is not valid.', 'bookly' ) . ' (' . $_SESSION['google_calendar_error'] . ')';
        ...
        $this->render( 'edit', compact( 'errors' ) );
        exit;
    }
    ...
    {
        $collection = AB_Holiday::query( 'h' )->where( 'h.staff_id', $id )->fetchArray();
        $holidays = array();
        if ( count( $collection ) ) {
