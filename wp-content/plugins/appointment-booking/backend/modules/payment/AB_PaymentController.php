...
class AB_PaymentController extends AB_Controller
{
    public function index()
    {
        ...
        $types     = array(
            AB_Payment::TYPE_LOCAL,
            AB_Payment::TYPE_2CHECKOUT,
            AB_Payment::TYPE_PAYPAL,
            AB_Payment::TYPE_AUTHORIZENET,
            AB_Payment::TYPE_STRIPE,
            AB_Payment::TYPE_COUPON,
        );
        $providers = array();
        $services  = array();
        foreach ( AB_Staff::query()->select( 'full_name' )->fetchArray() as $staff ) {
            $providers[] = $staff['full_name'];
        }
        foreach ( AB_Service::query()->select( 'title' )->fetchArray() as $service ) {
    ...
    private function createQuery( $request )
    {
        ...
        if (
            ...
            $query->sortBy( $request['order_by'] )
                ->order( $request['sort_order'] == 'desc' ? AB_Query::ORDER_DESCENDING : AB_Query::ORDER_ASCENDING );
        }
