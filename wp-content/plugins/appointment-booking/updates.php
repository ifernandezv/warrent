...
class AB_Updates
{
    function update_7_7()
    {
        ...
        $wpdb->query( "ALTER TABLE " . AB_Payment::getTableName() . " CHANGE COLUMN `type` `type` ENUM('local','coupon','paypal','authorizeNet','stripe','2checkout') NOT NULL DEFAULT 'local'" );
        $wpdb->query( "ALTER TABLE " . AB_Payment::getTableName() . " CHANGE COLUMN `transaction` `transaction_id` VARCHAR(255) NOT NULL" );
...
function bookly_plugin_update_db()
{
    ...
    if ( AB_Plugin::version > $db_version ) {

        $db_version_underscored     = 'update_' . str_replace( '.', '_', $db_version );
        $plugin_version_underscored = 'update_' . str_replace( '.', '_', AB_Plugin::version );
        ...
        update_option( 'ab_db_version', AB_Plugin::version );
    }
}