...
if(!class_exists('AIO_Icon_Manager'))
{
	class AIO_Icon_Manager
	{
		...
		function __construct()
		{
			...
			if ( function_exists('add_shortcode_param'))
			{
				add_shortcode_param('chk-switch' , array($this, 'checkboxes_param'), plugins_url('../admin/vc_extend/js/ultimate-params.js',__FILE__)) ;
			}		
		}
		function button_prev_param($settings, $value){
			...
			$json_file = isset($settings['json_file']) ? $settings['json_file'] : '';
		...
		function animator_param($settings, $value){
			...
			*/
			$json_file = plugins_url('../assets/js/animate-config.json',__FILE__);
			$json = file_get_contents($json_file);
			$jsonIterator = json_decode($json,true);
