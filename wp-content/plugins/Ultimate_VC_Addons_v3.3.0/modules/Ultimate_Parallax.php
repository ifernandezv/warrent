...
if(!class_exists('VC_Ultimate_Parallax')){
	class VC_Ultimate_Parallax{
		...
		public static function parallax_shortcode($atts, $content){
			...
			$bg_type = $bg_image = $bg_image_new = $bsf_img_repeat = $parallax_style = $video_opts = $video_url = $video_url_2 = $video_poster = $bg_image_size = $bg_image_posiiton = $u_video_url = $parallax_sense = $bg_cstm_size = $bg_override = $bg_img_attach = $u_start_time = $u_stop_time = $layer_image 			= $css = $animation_type = $horizontal_animation = $vertical_animation = $animation_speed = $animated_bg_color = $fadeout_row = $fadeout_start_effect = $fadeout_no_effect_classes = $parallax_content = $parallax_content_sense = $disable_on_mobile = "";
			...
			if($bg_type== "no_bg"){
				$html .= '<div class="upb_no_bg" data-fadeout="'.$fadeout_row.'" data-fadeout-percentage="'.$fadeout_start_effect.'" data-fadeout-disable="'.$fadeout_no_effect_classes.'" data-parallax-content="'.$parallax_content.'" data-parallax-content-sense="'.$parallax_content_sense.'" data-row-effect-mobile-disable="'.$disable_on_mobile.'"></div>';
			}	
		...
		function parallax_init(){
			...
			if(function_exists('vc_add_param')){
				...
				vc_add_param('vc_row',array(
						"type" => "textfield",
						"class" => "",
						"heading" => __("Exclude Classes", "upb_parallax"),
						"param_name" => "fadeout_no_effect_classes",
						//"admin_label" => true,
						"value" => "",
						"group" => $group_effects,
						"description" => __("Optionally, you may exclude CSS classes (separated by comma) from fade out effect.", "upb_parallax")
					)
				);
				vc_add_param('vc_row',array(
						"type" => "chk-switch",
						"class" => "",
						"heading" => __("Disable on Mobile", "upb_parallax"),
