Nictitate theme
--------------------

Nictitate is a multi purpose wordpress theme which could be used for almost all kinds of business. You can use it for Ecommerce, Blog, Portfolio, Coporate web site. Especially, it comes with Jigoshop plugin which makes it become a good ecommerce website. Fully responsive Nictitate looks beautiful and is very attractive on any device. It has two optional versions – Wide and Boxed and multi-layouts, this Multi Purpose WordPress Theme provides everything you’ll need to create a professional looking website. Theme is designed and coded by http://kopatheme.com

--------------------
Sources and Credits
--------------------

--------------------
Font Awesome License
--------------------

Font License - http://fontawesome.io
License: SIL OFL 1.1
License URI: http://scripts.sil.org/OFL
Copyright: Dave Gandy, http://fontawesome.io

Code License - http://fontawesome.io
License: MIT License
License URI: http://opensource.org/licenses/mit-license.html
Copyright: Dave Gandy, http://fontawesome.io

Brand Icons
All brand icons are trademarks of their respective owners.
The use of these trademarks does not indicate endorsement of the trademark holder by Font Awesome, nor vice versa.

--------------------
Javascript Licenses
--------------------
* bootstrap-tooltip.js, bootstrap.js
	- http://twitter.github.com/bootstrap/javascript.html#tooltips
 	- Licensed under the Apache License, Version 2.0 (the "License");
* cbpAnimatedHeader.min.js
	- Copyright 2013, Codrops. http://www.codrops.com
  	- Licensed under the MIT license. http://www.opensource.org/licenses/mit-license.php
* classie.js
	- from bonzo https://github.com/ded/bonzo
	- Bonzo is Copyright © 2014 Dustin Diaz @ded and licensed under the MIT licence
* css3-mediaqueries.js
	- CSS3 Media Queries | MIT Licensed
	- Source: https://code.google.com/p/css3-mediaqueries-js
* custom.js
	- Copyright kopatheme - http://kopatheme.com
	- Licensed under the MIT license. http://www.opensource.org/licenses/mit-license.php
* html5.js
	- HTML5 Shiv v3.6 | @afarkas @jdalton @jon_neal @rem 
	- MIT/GPL2 Licensed
	- Source: https://github.com/aFarkas/html5shiv 
* jflickrfeed.min.js
	- Copyright (C) 2009 Joel Sutherland
	- Licenced under the MIT license
	- http://www.newmediacampaigns.com/page/jquery-flickr-plugin
* jquery.carouFredSel-6.0.4-packed.js
	- Copyright (c) 2012 Fred Heusschen, www.frebsite.nl
	- Dual licensed under the MIT and GPL licenses.
	http://en.wikipedia.org/wiki/MIT_License
	http://en.wikipedia.org/wiki/GNU_General_Public_License
* jquery.flexslider-min.js, jquery.flexslider.js
	- Copyright 2012 WooThemes
	- Free to use under the GPLv2 license. http://www.gnu.org/licenses/gpl-2.0.html
* jquery.form.js
	- Copyright 2006-2013 (c) M. Alsup
	- Dual licensed under the MIT and GPL licenses:
	http://www.opensource.org/licenses/mit-license.php
	http://www.gnu.org/licenses/gpl.html
* jquery.hoverdir.js
	- Licensed under the MIT license. http://www.opensource.org/licenses/mit-license.php
	- Copyright 2012, Codrops
* jquery.hoverIntent.js
	- Copyright 2007, 2013 Brian Cherne
	- Licensed under the MIT license. http://www.opensource.org/licenses/mit-license.php
* jquery.prettyPhoto.js
	- Copyright Stephane Caron (http://www.no-margin-for-errors.com)
  	- prettyPhoto is totally free to use, it is released the GPLv2 (http://www.gnu.org/licenses/gpl-2.0.html) or Creative Commons 2.5 license (http://creativecommons.org/licenses/by/2.5/.
* jquery.validate.min.js
	- Copyright (c) 2006 - 2011 Jörn Zaefferer
	- Dual licensed under the MIT and GPL licenses:
	http://www.opensource.org/licenses/mit-license.php
	http://www.gnu.org/licenses/gpl.html
* modernizr.custom.js
	-  Modernizr 2.6.2 (Custom Build) | MIT & BSD
	- Licensed under the MIT license. http://www.opensource.org/licenses/mit-license.php
	- Build: http://modernizr.com/download/#-generatedcontent-touch-shiv-cssclasses-teststyles-prefixes-load
* PIE.htc
	- CSS3 rendering for IE Version 1.0.0, http://css3pie.com
	- Dual licensed for use under the Apache License Version 2.0 or the General Public License (GPL) Version 2.
* retina.js
	- Github: https://github.com/imulus/retinajs
	- License under the MIT license
* sequence.jquery-min.js, sequence.jquery.js
	- Author URL: http://www.ianlunn.co.uk/, https://github.com/IanLunn/Sequence
	- This is a FREE script and is available under a MIT License: http://www.opensource.org/licenses/mit-license.php
* superfish.js
	- Copyright (c) 2013 Joel Birch
	- Dual licensed under the MIT and GPL licenses:
	http://www.opensource.org/licenses/mit-license.php
	http://www.gnu.org/licenses/gpl.html

--------------------
PHP Licenses
--------------------
* TGMPluginActivation.class.php
    - Copyright 2011 Thomas Griffin (thomasgriffinmedia.com)
    - Author URI:  http://tgmpluginactivation.com/
    This program is free software; you can redistribute it and/or modify
    it under the terms of the GNU General Public License, version 2, as
    published by the Free Software Foundation.

--------------------
Other Licenses
--------------------
See headers of files for further details.

--------------------
Screenshot
--------------------
http://pixabay.com/en/car-mercedes-automobile-78738/


Thanks!
KOPASOFT
